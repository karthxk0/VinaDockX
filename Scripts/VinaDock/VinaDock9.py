...
======================================================================                                                       
 ...
 \ \    / (_)           |  __ \           | |   
  ...
  > Version 2.9  |  > Built by karthxk (https://karthxk0.github.io/)         
...
def split_models(split_exe, pdbqt_file, out_dir_pdbqt, out_dir_sdf, receptor_base, lig_base, space_tag, flex_tag):
    ...
    unique_folder_name = f"{lig_base}-{receptor_base}{space_tag}{flex_tag} Split Models"
    ...
    for sf in split_files:
        ...
        new_name_base = f"{lig_base}-{receptor_base}-Model{model_num}{space_tag}{flex_tag}_VinaDock_Output"
        ...
        if os.path.exists(flex_file):
            new_flex_name = f"{lig_base}-{receptor_base}-Model{model_num}{space_tag}-FlexRes_VinaDock_Output.pdbqt"
...
def generate_merged_log(log_files, output_dir, receptor_name, merge_filename, mode_str, search_space_full):
    ...
    with open(merged_path, 'w') as out_f:
        out_f.write("# Generated by VinaDock.py v2.9\n")
...
def main():
    ...
    for idx, (batch_ligands, lig_base) in enumerate(batches, 1):
        ...
        out_filename = f"{lig_base}-{receptor_base}{space_tag}{flex_tag}_VinaDock_Output.pdbqt"
        log_filename = f"{lig_base}-{receptor_base}{space_tag}{flex_tag}_VinaDock_Log.txt"
        ...
        if success:
            ...
            fail_count += 1
            failed_details.append(f"{lig_base} + {receptor_base} ({error})")
